...
namespace BibliotecaSubeta.Data
{
    /// <summary>
    /// Implementação da interface de repositorio presente na BibliotecaSubeta.Dominio.
    /// Tem por base o EntityFramework e interface IEntidade.
    /// </summary>
    /// <typeparam name="TContexto"></typeparam>
    public class Repositorio<TContexto> : IRepositorio, IDisposable where TContexto : DbContext
    {
        ...
        where TEntidade : class, IEntidade
        {
            ...
            if (incluirPropriedades.Length != 0)
            {
        ...
        public virtual void Registrar<TEntidade>(TEntidade entidade) where TEntidade : class, IEntidade
        {
            try
            {
                contexto.Set<TEntidade>().Attach(entidade);
        ...
        #region Querys
        public virtual IEnumerable<TEntidade> ObterTodos<TEntidade>(Expression<Func<TEntidade, bool>> filtro, Func<IQueryable<TEntidade>, IOrderedQueryable<TEntidade>> ordenarPor, int? skip, int? take, params Expression<Func<TEntidade, object>>[] incluirPropriedades) where TEntidade : class, IEntidade
        {
            return ObterQueryable<TEntidade>(filtro, ordenarPor, skip, take, incluirPropriedades).ToList();
        }

        public virtual async Task<IEnumerable<TEntidade>> ObterTodosAsync<TEntidade>(Expression<Func<TEntidade, bool>> filtro, Func<IQueryable<TEntidade>, IOrderedQueryable<TEntidade>> ordenarPor, int? skip, int? take, params Expression<Func<TEntidade, object>>[] incluirPropriedades) where TEntidade : class, IEntidade
        {
        ...
        public virtual TEntidade Obter<TEntidade>(Expression<Func<TEntidade, bool>> filtro, params Expression<Func<TEntidade, object>>[] incluirPropriedades) where TEntidade : class, IEntidade
        {
            return ObterQueryable<TEntidade>(filtro, null, incluirPropriedades: incluirPropriedades).SingleOrDefault();
        }

        public virtual async Task<TEntidade> ObterAsync<TEntidade>(Expression<Func<TEntidade, bool>> filtro, params Expression<Func<TEntidade, object>>[] incluirPropriedades) where TEntidade : class, IEntidade
        {
